...
export async function inflateAddr(
  contractName: string,
  addr: string,
  ...
  const factory = (await ethers.getContractFactory(contractName)).connect(
    authority
  );
...
export async function refContract(
  contractName: string,
  addr: string,
  authority?: Signer
): Promise<Contract> {
...
export async function validateDeploy(
  ...
  if (!isMatch) {
    ...
    console.log(
      `Success: Contract ${contractName} deployed at ${addr} matches source`
    );
  }
}
...
function explainMismatch(
  ...
  if (sourceBytecode.length - chainBytecode.length !== 0) {
    ...
    console.log(
      `Failure: Contract ${contractName} deployed at ${addr} differs from source in length (${lengthClause})`
    );
    return;
  }
  let byteMismatch;
  ...
  console.log(
    `Failure: Contract ${contractName} deployed at ${addr} differs from source at byte ${byteMismatch} of ${sourceBytecode.length}`
  );
}
...
async function injectCodeFromChain(
  address: string,
